...
Application.$controller('GooglemapsController', ['$scope', 'Utils', '$element', 'NgMap', '$timeout', '$http', 'wmToaster', 'CONSTANTS', '$compile',
    function ($s, Utils, $el, NgMap, $timeout, $http, wmToaster, CONSTANTS, $compile) {
        ...
        function constructMarkersModel() {
            ...
            _.forEach(_locations, function(marker, index) {
                ...
                $s.markersData.push({
                    ...
                    'id'            : $s.$id + '_' + index,
                    'color'         : $s.color ? Utils.findValueOf(marker, $s.color) : '',
        ...
        function assignLocations(dataset, options, columns, wp) {
            ...
            var propertiesMap = dataset && dataset.propertiesMap;
            ...
            if (WM.isArray(dataset)) {
                ...
                } else if (propertiesMap && propertiesMap.columns) {
                    _.forEach(propertiesMap.columns, function(column){
            ...
            if ($s.widgetid) {
                ...
                if (_locations.length > 0) {
                    Utils.getAllKeysOf(_locations[0]).forEach(function (key) {
                        options.push(key);
                    });
                } else {
        ...
        function mapTypeOperations(newVal) {
            if ($s.widgetid) {
                var wp = $s.widgetProps,
                    ...
                    commonProps  = ['name', 'tabindex', 'maptype', 'zoom', 'height', 'width', 'show', 'animation', 'onLoad', 'onDestroy'],
                    maptypeProps;
                if (newVal === 'Markers') {
